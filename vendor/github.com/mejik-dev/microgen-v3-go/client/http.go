...
func (h *HTTPClient) sendRequest(method string, url string, body io.Reader, headers map[string]string) (*HTTPResponse, *MicrogenError) {
	client := http.Client{}
...
func (h *HTTPClient) Get(path string, headers map[string]string) (*HTTPResponse, *MicrogenError) {
	return h.sendRequest(http.MethodGet, path, nil, headers)
}

func (h *HTTPClient) Post(path string, rawBody interface{}, headers map[string]string) (*HTTPResponse, *MicrogenError) {
	body := h.parseBody(rawBody, headers)
	return h.sendRequest(http.MethodPost, path, body, headers)
}

func (h *HTTPClient) Patch(path string, rawBody interface{}, headers map[string]string) (*HTTPResponse, *MicrogenError) {
	body := h.parseBody(rawBody, headers)
	return h.sendRequest(http.MethodPatch, path, body, headers)
}

func (h *HTTPClient) Delete(path string, headers map[string]string) (*HTTPResponse, *MicrogenError) {
	return h.sendRequest(http.MethodDelete, path, nil, headers)
}

func (h *HTTPClient) Send(method string, path string, rawBody interface{}, headers map[string]string) (*HTTPResponse, *MicrogenError) {
	body := h.parseBody(rawBody, headers)
